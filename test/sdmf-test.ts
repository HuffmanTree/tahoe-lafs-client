...
describe('TahoeLAFSClient (SDMF)', function () {
  ...
  before(function () {
    ...
    return http.put<string>(`/uri?format=${Format.SDMF}`, form1)
      .then(response => {
	filecapSDMF = response.data;

	return http.post<string>(`/uri?t=mkdir-with-children&format=${Format.SDMF}`, { 'foo.txt': ['filenode', { ro_uri: filecapSDMF }] })
      })
      .then(response => {
	dircapSDMF = response.data;
  ...
  describe('createDirectory', function () {
    ...
    it('should successfully create a directory with contents', async function () {
      ...
      const filenode: [string, FilecapInfo] = ['filenode', { ro_uri: filecapSDMF }];
      const dirnode: [string, DircapInfo] = ['dirnode', { ro_uri: dircapSDMF }];
      const response = await client.createDirectory(Format.SDMF, {
	'file1.txt': filenode,
	'folder1': dirnode
      });
